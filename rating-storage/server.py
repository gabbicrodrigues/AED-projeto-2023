...
def get_page():
    ...
    btree = BTree.load_from_file('rating_btree.json')
...
def save_page():
    ...
    btree.save_to_file('rating_btree.json')
...
def serve_binary_file():
    file_path = os.path.join(os.getcwd(), 'rating_btree.json')
...
def upload_file():
    try:
        ...
        with open('rating_btree.json', 'w', encoding='utf-8') as file:
            json.dump(data, file)
